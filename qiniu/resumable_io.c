	...
	Qiniu_Client* c, Qiniu_Rio_PutRet* ret, const char* key, Qiniu_Int64 fsize, Qiniu_Rio_PutExtra* extra)
{
	...
	//// For using multi-region storage.
	{
		if ((upHost = extra->upHost) == NULL) {
			if (Qiniu_Rgn_IsEnabled()) {
				if (extra->upBucket && extra->accessKey) {
					err = Qiniu_Rgn_Table_GetUpHost(c->regionTable, c, extra->upBucket, extra->accessKey, extra->upHostFlags, &upHost, &upHostVote);
				} else {
					err = Qiniu_Rgn_Table_GetUpHostByUptoken(c->regionTable, c, extra->uptoken, extra->upHostFlags, &upHost, &upHostVote);
				} // if
				if (err.code != 200) {
					return err;
				} // if
